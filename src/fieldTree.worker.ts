...
self.onmessage = (e) => {
    ...
    fields.forEach((field: string, field_index: number) => {
        // Post progress every 100 fields or at the very end to avoid too many messages.
        if (field_index % 100 === 0 || field_index === total - 1) {
            self.postMessage({ type: 'progress', data: (field_index + 1) / total * 100.0 });
        }

        if (!field || field_index < ignoreFieldNum) return;
